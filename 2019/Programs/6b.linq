...
I)SAN".Split('\n');
	...
	var me = AllPlanets["YOU"];
	var santa = AllPlanets["SAN"];	
	
	List<Planetoid> santasParents = new List<Planetoid>();
	List<Planetoid> myParents = new List<Planetoid>();
	
	PopulateParents(santasParents, santa.Parent);
	PopulateParents(myParents, me.Parent);
	
	var commonParents = santasParents.Intersect(myParents);

	//commonParents.Dump();

	var mostRecentCommonAncestor = commonParents.First();
	
	var distance = (me.Distance - mostRecentCommonAncestor.Distance - 1) + (santa.Distance - mostRecentCommonAncestor.Distance - 1);
...
void PopulateParents(List<Planetoid> parentList, Planetoid planet)
{
	parentList.Add(planet);
	if (planet.Parent != null)
		PopulateParents(parentList, planet.Parent);
	return;
}
